...
const WeatherDetails = () => {
  ...
  const saveLocation = (weatherData) => {
    ...
    const weatherDataExists = existingWeather.some(
      (data) => data.id === weatherData[0].id
    );
  ...
  return (
    ...
    >
      ...
      <div className=" relative flex flex-col h-full py-10 items-center justify-center">
        <div
          className="relative shadow-2xl lg:w-[60%] w-[90%] lg:p-10 px-5 py-5 mx-auto text-white rounded-lg"
          style={{
            backdropFilter: "blur(10px)",
            background: "rgba(184, 182, 182, 0.15)",
          }}
        >
          <div className="bg-white text-black p-2 rounded-full w-[65px]">
            <Link to="/">
              ...
              return (
                <div key={id}>
                  <div className="lg:flex text-center lg:text-left items-center justify-between py-6 font-semibold">
                    ...
                    <div>
                      <button
                        onClick={() => saveLocation(weatherDetails)}
                        className="bg-blue-500 hover:shadow-lg py-3 px-8 text-white hover:bg-blue-700 duration-700 transition-all ease-in-out"
                      >
                        Save Location
                      </button>
                    </div>
